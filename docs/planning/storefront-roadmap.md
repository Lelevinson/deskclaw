...
**Status:** DECIDED (2026-06-02) — scope, stack, and styling chosen in the planning session below. The MVP **skill** backlog is closed ([`skill-roadmap.md`](skill-roadmap.md) §4); the storefront was the one remaining in-scope MVP item. ARCHITECTURE §2/§3/§5/§6 were updated to match this decision. **Build progress (§6): COMPLETE (2026-06-03).** All six phases merged — Phase 1 Design (PR #12), Phase 2 Foundation (PR #14), Phase 3 Cart (PR #16), Phase 4 Orders (PR #18, docs #19), Phase 5 Returns (PR #20), and Phase 6 Polish (PR #21). The storefront is built; there are **no remaining in-scope MVP items.**
...
5. ~~**Returns** (`feat/storefront-returns`)~~ — **DONE (merged 2026-06-03, PR #20).** Read-only own-returns-only returns list + detail (surface 6). `getReturns()`/`getReturn(id)` reuse the chat tools' `listReturnsForChannel`/`getReturnForChannel`; unknown OR non-owned id → neutral not-found (no existence leak). Never renders a refund/money action (a refund is a human/seeded outcome shown read-only); the delivered-order "Request a return" CTA stays inert (opening a return is a write — out of the cart-only-mutation scope) and links to the live `/returns` list. See `web/README.md` "Returns".
6. ~~**Polish** (`feat/storefront-polish`)~~ — **DONE (merged 2026-06-03, PR #21). This closes §6 — the storefront is complete.** Cross-cutting pass, no new routes/domains/mutations: responsive shell (new `MobileNav` hamburger collapse fixing the header/wordmark overlap), shared states (`Skeleton` + `loading.tsx` per route, reduced-motion shimmer; neutral `error.tsx` with no existence leak), the deferred shared-primitive cleanups (`StatusPill` under the three badges, `LineItemRow` under cart + order rows), an accessibility pass (shared `.focus-ring`, keyboard-operable mobile menu), and the brand-finish favicon (gold monogram-A-in-laurel vector `app/icon.svg`). See `web/README.md` "Polish".
