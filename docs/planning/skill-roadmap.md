...
**Dependency graph:** ~~`cart-edit`~~ (done) → `eval-harness` → `order-status` → `returns-intake`. `handoff-ticket` and `product-compatibility` are independent and can land in any gap. `storefront UI` (deferred) depends on `order-status` existing.
