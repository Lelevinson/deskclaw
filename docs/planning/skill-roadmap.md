...
**Dependency graph:** ~~`cart-edit`~~ → ~~`eval-harness`~~ → ~~`order-status`~~ → ~~`returns-intake`~~ → ~~`handoff-ticket`~~ (all done). `product-compatibility` is independent and is the remaining open item. `storefront UI` (deferred) now has its prerequisite — the `orders` domain exists (note: `handoffs` is staff/ops-only, so it is **not** part of that customer-visible storefront scope — ARCHITECTURE §6).
