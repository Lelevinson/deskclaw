...
- `cart-actions` — add/remove/update cart, identity-gated, with preview → confirm → audit
- `order-status` — read-only order lookup, identity-gated by linked channel account
...
Backend: the shop MCP (`shop_*` tools) over a local JSON store, reset from `data/`. The reusable mutation pipeline is identity (account link) → preview → explicit confirm → execute → audit log; order-status uses the same identity gate for read-only order access.
...
| Local demo UI / storefront smoke render | **BUILT after orders (UI, not a skill)** | Governed by ARCHITECTURE §6. Implemented as an interactive local browser demo via `npm run demo:server` plus a generated read-only state snapshot via `npm run shop:storefront` — not per skill. |
...
1. ✅ **cart-edit** (`cart-actions` extension) — "remove this" / "make it 2 instead". *Extension.* Tools: `shop_cart_preview_remove_item` / `_update_quantity` + matching confirm tools (by-id and confirm-latest); extended `PendingAction.type` union ([`types.ts`](../../src/shop/types.ts)). Data: none new (reuses `carts`). Scope: no new domain — fit current scope. **SHIPPED 2026-05-29** (branch `feat/cart-edit`): proved the identity → preview → confirm → execute → audit pipeline generalizes to new mutation types; quantity→0 routes to remove (not an update), over-stock and not-in-cart are refused, and the eval harness now exercises all three `PendingAction` types. **All sequential backlog items through returns-intake are now shipped.**
2. ✅ **eval-harness (tool-level)** — not a customer skill; a safety net. *Inner-tooling / test infra.* Deterministic tests over `src/shop` service functions (identity gating, preview/confirm, cart expiry, audit logging) run from a seeded DB. Data: reuses `data/` baseline. Scope: closes the §3 "not implemented" gap; no product-scope change. **SHIPPED 2026-05-29** (branch `feat/eval-harness`): `src/cli/shop-eval.ts`, run via `npm run shop:eval` (`shop:test` is now an alias). It includes named assertions across identity/ownership (unlinked, revoked, typed-id-not-proof, cross-customer and cross-link confirm rejection), preview→confirm (no-mutation for all three types, confirm-required, no double-execution), cart expiry, per-type refusals, audit logging for all three `PendingAction` types, and identity-gated order/return reads and handoff ticket creation; per-test `data/` reset, non-zero exit on failure, clean DB at end. The standalone smoke test was retired and folded in. Skill/agent-layer automation stays manual per §5. See §"Eval harness" below. **Order: 2 — done before the first new data domain.**
3. ✅ **order-status** (new `order-status` skill) — "where's my order?". *New skill + new data domain.* Tools: `shop_orders_list_for_channel`, `shop_order_get` (read-only, identity-gated). Data: **new `data/shop/orders.json`** (orders owned by `customerId`, with status + line items + tracking fields). Scope: new visible data domain. **SHIPPED 2026-05-29** (branch `feat/order-status`): seeded orders, identity-gated read tools, order-status skill instructions, scenarios, and tool-level eval assertions that order numbers are filters rather than ownership proof. **All sequential backlog items through returns-intake are now shipped.**
4. ✅ **returns-intake** (new `returns-actions` skill) — "I want to return / exchange this". *New skill, depends on `orders`.* Tools: `shop_return_preview` / `shop_return_confirm` (creates a return *request*, not a refund); reuses identity + preview→confirm→audit; auto-handoff for the money movement. **Plus a refund/return-status read** (`shop_return_status` — "is my refund approved / processed yet?"): read-only, identity-gated, safe by construction, over a `status` field on each return record. This is a *read folded into this skill*, not a separate skill, and is distinct from `policy-oracle` answering the generic "5–7 business days" timeframe. Data: **new `data/shop/returns.json`** (with per-return `status`) + return-window facts in `data/policies/returns.md`. Scope: new sub-domain. **SHIPPED 2026-05-30** (branch `feat/finish-mvp-backlog`): return request preview/confirm/status tools, seeded `returns` data, `returns-actions` skill, scenarios, and tool-level eval assertions. **Order: 4 (after order-status).**
5. ✅ **handoff-ticket** (`sentiment-router` extension) — durable escalation record. *Extension + one inner tool.* Tool: `shop_handoff_create` appending a handoff/ticket record; optional identity. Data: handoff records (new `data/shop/handoffs.json` or an action-log type). Scope: small; record-keeping for an existing behavior. **SHIPPED 2026-05-30** with `shop_handoff_create` and `data/shop/handoffs.json`. **Order: parallelizable — independent of orders.**
6. ✅ **product-compatibility** (`policy-oracle` extension) — ingredient/routine compatibility answers. *Extension, data-only.* Tools: none. Data: **new `data/catalog/compatibility.md`** (safe, brand-authored pairings/avoid-with facts). Scope: no new tool/pipeline; reuses the answer-only-from-data contract. **SHIPPED 2026-05-30** with `data/catalog/compatibility.md` and policy-oracle instructions. **Order: parallelizable — independent of everything above.**

**Dependency graph:** ~~`cart-edit`~~ (done) → ~~`eval-harness`~~ (done) → ~~`order-status`~~ (done) → ~~`returns-intake`~~ (done). ~~`handoff-ticket`~~ and ~~`product-compatibility`~~ are also done. The local browser demo is available via `npm run demo:server`; the static storefront snapshot is available via `npm run shop:storefront`.
...
**The tool-level harness shipped before the first new data domain. Full skill/agent scenario automation remains deferred.**
...
Sequencing note: `cart-edit` and the tool-level harness shipped before `order-status`, so the first new data domain and PII-bearing read now have deterministic identity-gating coverage.
...
- The tool-level eval harness is shipped (`npm run shop:eval`); keep skill/agent model-in-the-loop scenarios manual for now unless a future scoped task explicitly prioritizes automation.
