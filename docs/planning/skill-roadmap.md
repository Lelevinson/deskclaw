...
**Dependency graph:** ~~`cart-edit`~~ → ~~`eval-harness`~~ → ~~`order-status`~~ → ~~`returns-intake`~~ → ~~`handoff-ticket`~~ → ~~`product-compatibility`~~ (all done — `product-compatibility` was independent of the chain). **The MVP skill backlog is now complete.** The only remaining MVP item is `storefront UI` (deferred), which now has its prerequisite — the `orders` domain exists (note: `handoffs` is staff/ops-only, so it is **not** part of that customer-visible storefront scope — ARCHITECTURE §6).
