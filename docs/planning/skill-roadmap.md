...
**Dependency graph:** ~~`cart-edit`~~ → ~~`eval-harness`~~ → ~~`order-status`~~ → ~~`returns-intake`~~ (all done). `handoff-ticket` and `product-compatibility` are independent and can land in any gap — they are the remaining open items. `storefront UI` (deferred) now has its prerequisite — the `orders` domain exists.
