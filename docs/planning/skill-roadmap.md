...
4. **returns-intake** (new `returns-actions` skill) — "I want to return / exchange this". *New skill, depends on `orders`.* Tools: `shop_return_preview` / `shop_return_confirm` (creates a return *request*, not a refund); reuses identity + preview→confirm→audit; auto-handoff for the money movement. Data: **new `data/shop/returns.json`** + return-window facts in `data/policies/returns.md`. Scope: new sub-domain — update ARCHITECTURE. **Order: 4 (after order-status).**
