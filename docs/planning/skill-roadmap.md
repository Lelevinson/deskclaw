...
**Dependency graph:** ~~`cart-edit`~~ → ~~`eval-harness`~~ → ~~`order-status`~~ (all done) → `returns-intake` (next). `handoff-ticket` and `product-compatibility` are independent and can land in any gap. `storefront UI` (deferred) now has its prerequisite — the `orders` domain exists.
