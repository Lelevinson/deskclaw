...
2. `npm run shop:eval` — build + the tool-level eval harness. It runs against a temp sandbox DB (it does **not** touch `.local/shop-db.json`, so it is safe during a live session), resetting from the `data/` baseline per test, and runs every identity, ownership, preview→confirm, expiry, refusal, and audit-log assertion across catalog search, all three cart action types, the read-only order-status and returns-intake flows, and the append-only handoff/escalation records (optional identity, no preview/confirm). Each assertion prints a named PASS/FAIL; the run exits non-zero if any fails. (`npm run shop:test` is an alias for the same harness.)
