...
- **Out-of-band verification-code delivery and link *repair*** — self-service registration + code-gated linking of an existing account **shipped 2026-06-09** (in scope, above). What remains deferred: *delivering* a one-time code out-of-band (we have no async outbound channel — same blocker as restock alerts), and repairing a **revoked** link from chat. Linking a pre-existing account **without** a verification code stays out of scope (the unverified account-takeover path). The demo uses a per-account `accountCode` as the code stand-in.
