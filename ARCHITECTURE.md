...
- Mock e-commerce website UI — **build underway** (the one remaining in-scope MVP item now that the skill backlog is closed). Interactive companion storefront on the existing `src/shop` backend (no checkout), Next.js + Tailwind + shadcn/ui; plan/phasing in [`docs/planning/storefront-roadmap.md`](docs/planning/storefront-roadmap.md) §6, scope in §5/§6 below. **Progress:** Phase 1 Design (`web/DESIGN.md`, PR #12), Phase 2 Foundation (Next.js scaffold + `src/shop` reuse layer + app shell + catalogue/PDP vertical slice, PR #14), and Phase 3 Cart (cart surface + audited add/remove/update reusing the cart service, PR #16) are **merged**; Phase 4 Orders (read-only own-orders-only order history + detail/tracking reusing `listOrdersForChannel`/`getOrderForChannel`, Orders nav live) is **built (PR pending review)**; returns/polish surfaces (Phases 5–6) are not yet built. The Phase 3 cart resolved the open **UI confirm-vs-audit** question (roadmap §3/§8): server actions reuse the chat `preview→confirm` path so cart mutations still write the audit log (the click is the consent) — see [`web/README.md`](web/README.md) "Cart mutations".
