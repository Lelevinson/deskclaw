...
- Mock e-commerce website UI — **build underway** (the one remaining in-scope MVP item now that the skill backlog is closed). Interactive companion storefront on the existing `src/shop` backend (no checkout), Next.js + Tailwind + shadcn/ui; plan/phasing in [`docs/planning/storefront-roadmap.md`](docs/planning/storefront-roadmap.md) §6, scope in §5/§6 below. **Progress:** Phase 1 Design (`web/DESIGN.md`, PR #12) and Phase 2 Foundation (Next.js scaffold + `src/shop` reuse layer + app shell + catalogue/PDP vertical slice, PR #14) are **merged**; cart/orders/returns surfaces (Phases 3–6) are not yet built.
