...
3. If unlinked, ask whether they want to **create a new account** or **link an existing account**:
   - **New account:** ask the name they want on the account, read it back to confirm, then call `shop_account_register` with the channel identity and that name. On success, welcome them and continue with whatever they originally wanted.
