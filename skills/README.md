...
- [`routine-concierge/`](routine-concierge/) — builds a conversational AM/PM routine from one natural-language ask (need + budget): selects products from the catalog, orders/pairs them only from `data/catalog/compatibility.md`, then offers to add the whole bundle to the cart via the normal per-item preview→confirm path (no bulk-add tool, no auto-add). The tool-chaining counterpart to the deterministic public `/routines` web page.
